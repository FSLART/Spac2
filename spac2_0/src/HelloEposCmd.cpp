...
int Demo(unsigned int* p_pErrorCode, int actuator_angle){

	int lResult = MMC_SUCCESS;
	unsigned int lErrorCode = 0;
	long value; 
	//void* KeyHandle, unsigned short NodeId, long* pTargetPosition, unsigned int* pErrorCode
	lResult = VCS_GetTargetPosition(g_pKeyHandle, g_usNodeId, &value, &lErrorCode);
...
int actuate(int actuator_angle){
	...
	if((lResult = Demo(&ulErrorCode, actuator_angle))!=MMC_SUCCESS)
	{
